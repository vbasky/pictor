...
//!
//! Implements `y = W × x` where W is stored as Q3_K blocks.
//! Each super-block covers 256 weights (QK_K = 256).

use pictor_core::BlockQ3K;

use crate::error::{KernelError, KernelResult};

/// Scalar Q3_K GEMV: computes `output = weight_matrix × input`.
///
/// The weight matrix `W` is stored in row-major Q3_K format:
/// row `i` starts at block index `i * blocks_per_row` where
/// `blocks_per_row = in_features / 256`.
///
/// # Parameters
///
/// - `blocks`:      Q3_K-quantized weight blocks in row-major order.
...
pub fn gemv_q3k(
    ...
    for row in 0..n_rows {
        ...
        BlockQ3K::dequant(row_blocks, &mut row_buf).map_err(KernelError::Core)?;
...
mod tests {
    ...
    fn make_q3k_block(value: f32) -> BlockQ3K {
    ...
    #[test]
    fn gemv_q3k_single_row_uniform() {
        // One row, uniform weight = 1.0, input all 1.0.
        // Expected output ≈ 256.0 (with quantization error < 20%).
        let block = make_q3k_block(1.0);
        let input = vec![1.0f32; 256];
        let mut output = vec![0.0f32; 1];

        gemv_q3k(&[block], &input, &mut output, 1, 256).expect("gemv ok");
        assert!(
            output[0] > 0.0,
            "output should be positive for positive weights, got {}",
            output[0]
        );
    }

    #[test]
    fn gemv_q3k_two_rows() {
        let block_pos = make_q3k_block(0.5);
        let block_neg = make_q3k_block(-0.5);
        let input = vec![1.0f32; 256];
        let mut output = vec![0.0f32; 2];

        gemv_q3k(&[block_pos, block_neg], &input, &mut output, 2, 256).expect("gemv ok");
        assert!(
            output[0] > 0.0,
    ...
    #[test]
    fn gemv_q3k_not_block_aligned_errors() {
        let block = make_q3k_block(1.0);
        let input = vec![1.0f32; 100];
        let mut output = vec![0.0f32; 1];
        assert!(
            gemv_q3k(&[block], &input, &mut output, 1, 100).is_err(),
            "should error when in_features not multiple of 256"
        );
    }

    #[test]
    fn gemv_q3k_wrong_block_count_errors() {
        let block = make_q3k_block(1.0);
        let input = vec![1.0f32; 256];
        let mut output = vec![0.0f32; 2];
        assert!(
            gemv_q3k(&[block], &input, &mut output, 2, 256).is_err(),
            "should error on block count mismatch"
        );
    }

    #[test]
    fn gemv_q3k_output_too_small_errors() {
        let block = make_q3k_block(1.0);
        let input = vec![1.0f32; 256];
        let mut output = vec![0.0f32; 0];
        assert!(
            gemv_q3k(&[block], &input, &mut output, 1, 256).is_err(),
